...
-- CREATE TEMP FUNCTION getData(inputBundle STRING)
select * from ((
select os, id  from (
select id, os, ARRAY_CONCAT_AGG(groupedData.day), CheckConsec(ARRAY_CONCAT_AGG(groupedData.day), 4) as whaa from (
...
from (select  STRUCT(id, os, total_counts, totalAmount, time) as data from (select * from(
...
FROM ((select parseRequest(requestUrl) as request, timestamp as time from (SELECT httpRequest.*, timestamp FROM `cs-686-lab06.loadBalancerLogs.loadBalancerLogs_large`)))
where request.amount > 0.0 AND request.bundle = 'id686'
) group by 1, 2, 3, 4)
-- order by 1 desc)
...
-- FROM `cs-686-lab06.loadBalancerLogs.loadBalancerLogs_parsed_medium`
FROM ((select parseRequest(requestUrl) as request, timestamp as time from (SELECT httpRequest.*, timestamp FROM `cs-686-lab06.loadBalancerLogs.loadBalancerLogs_large`)))
where request.amount > 0.0 AND request.bundle = 'id486'
) group by 1, 2, 3, 4)
-- order by 1 desc)
where total_counts >= 3
order by id))
